...
void Simplification::saveFile(vector<Point> &vert, vector<Face> &face)
{
	...
	for (int i = 0; i < (int)pointOut.size(); ++i) {
		...
		vert.push_back(Point(Vector3(pointSet.point[pointOut[i]].cdt.x, pointSet.point[pointOut[i]].cdt.y, pointSet.point[pointOut[i]].cdt.z)));
	}
	...
	for(iter = faceOut.begin(); iter != faceOut.end(); ++iter)
	{
		...
		face.push_back(Face((*iter).p[0], (*iter).p[1], (*iter).p[2]));
	}
